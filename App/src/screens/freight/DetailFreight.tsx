...
const DetailFreight = () => {
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg, paddingHorizontal: 16, paddingBottom: 20, paddingTop: 16 }} edges={["left", "right", "bottom"]} >

            <ScrollView showsVerticalScrollIndicator={false}>
                <View className="flex-row justify-between items-center">
                    <Text style={{ color: colors.text }} className="text-xl font-semibold">{freight.name}</Text>
                ...
                <Text style={{ color: colors.textSecondary }} className="text-sm font-semibold">{freight.origin_label}</Text>
                ...
                <View style={{ backgroundColor: colors.bgSecondary, borderColor: colors.bgTertiary, borderWidth: 1  }} className="p-4 rounded-xl gap-2.5 ">
                    ...
                    <View>
                        <Text style={{ color: colors.text }} className="text-sm font-semibold opacity-80">Origem: {freight.origin_label}</Text>
                        <Text style={{ color: colors.text }} className="text-sm font-semibold opacity-80">Destino: {freight.destination_label}</Text>
                        <Text style={{ color: colors.text }} className="text-sm font-semibold opacity-80">Prazo: {freight.daysLimit}</Text>
                    </View>
                </View>
                ...
                <View style={{ backgroundColor: colors.bgSecondary, borderColor: colors.bgTertiary, borderWidth: 1  }} className="p-4 rounded-xl gap-2.5 ">
                    ...
                    <View>
                        <Text style={{ color: colors.text }} className="text-sm font-semibold opacity-80">Empresa: Coamo</Text>
                        <Text style={{ color: colors.text }} className="text-sm font-semibold opacity-80">Tipo: Coorporativo</Text>
                    </View>
                </View>

                <Text style={{ color: colors.text }} className="text-base font-semibold py-5">{t("FREIGHT.MORE_DETAILS")}</Text>
                <DetailRow
                    label={t("FREIGHT.TYPE")}
                    value={freight.cargo?.name ?? t("CARD.ACTIVITY.N_A")}
                />
                <DetailRow
                ...
                />
                <DetailRow label={t("FREIGHT.DEADLINE")}
                    value={
