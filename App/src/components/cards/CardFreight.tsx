    ...
    <TouchableOpacity className="w-full p-2.5 rounded-2xl bg-lightBgNonary dark:bg-darkBgNonary border border-lightBgTertiary dark:border-darkBgTertiary" >
      ...
      <View className="flex-row justify-between pt-1 w-full gap-2">
        <View className="flex-col flex-1 gap-1.5">
          <Text className="text-lightTextSecondary dark:text-darkTextSecondary text-sm" numberOfLines={1}>Origem: {props.origin}</Text>
          <Text className="text-lightTextSecondary dark:text-darkTextSecondary text-sm" numberOfLines={1}>Destino: {props.destination}</Text>
          <Image source={require('../../assets/Logos.png')} className="max-w-auto h-auto pb-" resizeMode="contain" />
          <Text className="text-lightText dark:text-darkText font-semibold text-sm">Frete: R$ {props.freightValue}</Text>
        </View>
