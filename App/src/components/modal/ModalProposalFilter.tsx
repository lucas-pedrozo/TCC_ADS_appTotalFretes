...
type ModalProposalFilterProps = {
	visible: boolean;
	onClose: () => void;
	onApply: (filter: ProposalStatusFilter) => void;
	appliedFilter: ProposalStatusFilter;
};
...
function ModalProposalFilter({
	visible,
	onClose,
	onApply,
	appliedFilter,
}: ModalProposalFilterProps) {
	const colors = useThemeColors();
	const { mode } = useThemeMode();
	const { t } = useTranslation();
	const [draft, setDraft] = useState<ProposalStatusFilter>(appliedFilter);
					...
					<View className="rounded-t-2xl p-4" style={{ backgroundColor: colors.bg }}>
						<View className="flex-row justify-between items-center mb-4">
							<Text className="font-semibold text-lg flex-1 pr-2" style={{ color: colors.text }}>
								{t("MODALPROPOSALFILTER.TITLE")}
							</Text>
							<TouchableOpacity
								onPress={onClose}
								accessibilityRole="button"
								accessibilityLabel={t("MODALPROPOSALFILTER.CLOSE_A11Y")}
							>
						...
						<Text className="text-sm mb-4" style={{ color: colors.textSecondary }}>
							{t("MODALPROPOSALFILTER.SUBTITLE")}
						</Text>
						...
						<View>
							...
							>
								{t("MODALPROPOSALFILTER.STATUS")}
							</Text>
							<View className="flex-row flex-wrap gap-2">
								{PROPOSAL_STATUS_FILTER_OPTIONS.map((option) => (
									<FilterChip
										key={option}
										label={t(`MODALPROPOSALFILTER.STATUS_${option.toUpperCase()}`)}
										active={draft === option}
...
export default ModalProposalFilter;
